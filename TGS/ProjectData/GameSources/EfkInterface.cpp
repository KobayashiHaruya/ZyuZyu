...
namespace basecross {
	...
	void EfkPlay::SetRotation(Vec3 rot)
	{
		auto shptr = m_EfkInterface.lock();
		if (shptr && m_Handle != -1)
		{
			shptr->m_Manager->SetRotation(m_Handle, rot.x, rot.y, rot.z);
		}
	}

	void EfkPlay::Play(const shared_ptr<EfkEffect>& effect, const bsm::Vec3& Emitter) {
		auto iface = effect->m_EfkInterface.lock();
		if (iface) {
			m_Handle = iface->m_Manager->Play(effect->m_Effect, Emitter.x, Emitter.y, Emitter.z);
			m_EfkInterface = iface;
	...
	void EfkPlay::Play(const shared_ptr<EfkEffect>& effect, const bsm::Vec3& Emitter, const int32_t startFrame) {
		auto iface = effect->m_EfkInterface.lock();
		if (iface) {
			Effekseer::Vector3D emitter;
			emitter.X = Emitter.x;
			emitter.Y = Emitter.y;
			emitter.Z = Emitter.z;
			m_Handle = iface->m_Manager->Play(effect->m_Effect, emitter, startFrame);
			m_EfkInterface = iface;
		}
	}
	...
	void EfkInterface::OnCreate() {
		...
		// 描画用インスタンスの生成
		m_Renderer = ::EffekseerRendererDX11::Renderer::Create(pDx11Device, pID3D11DeviceContext, 5000);
		// エフェクト管理用インスタンスの生成
		m_Manager = ::Effekseer::Manager::Create(5000);
